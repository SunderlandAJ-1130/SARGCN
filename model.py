...
class SARGCNLayer(nn.Block):
    def __init__(self, in_feats, out_feats, num_rels, groups=2,
                 ...
                 activation=None, inter_channel=32, use_self_loop=False,
                 residual=True):
        super().__init__()
        self.radix = radix
    ...
    def forward(self, g, x, etypes, norm=None):
        ...
        # max pooling
        s = U_hat.reshape(U_hat.shape[0] // 80, 80, U_hat.shape[1], U_hat.shape[2]).mean(axis=1).reshape(-1,
                                                                                                         self.out_feats)
        # following Hang Zhang, we utilize two FC layers.
        a = nd.softmax(self.s2a2(self.s2a1(s)).reshape(s.shape[0], self.groups, self.radix, -1),
                       axis=2).expand_dims(axis=1)
        V = nd.broadcast_mul(a, U.reshape(U.shape[0] // 80, 80, U.shape[1], U.shape[2], U.shape[3])).sum(
            axis=3)
