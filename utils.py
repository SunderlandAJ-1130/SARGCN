...
def rmse_trainset(model, train_iter, batch_size, num, scaler):
    test = mx.nd.zeros(shape=(len(train_iter) * batch_size, num, 8), ctx=mx.gpu())
    pred = mx.nd.zeros(shape=(len(train_iter) * batch_size, num, 8), ctx=mx.gpu())
    count = 0
    ...
    return np.sqrt(MSE), MSE / 2


def rmse_testset(model, _iter, batch_size, num_entra, scaler, mae=False, idx=None, plot=False):
    test = mx.nd.zeros(shape=(len(_iter) * batch_size, num_entra, 8), ctx=mx.gpu())
    pred = mx.nd.zeros(shape=(len(_iter) * batch_size, num_entra, 8), ctx=mx.gpu())
    count = 0
    ...
    if mae:
        ...
        MAPE = masked_mape_np(test.asnumpy().flatten(), pred.asnumpy().flatten())
...
def collate(samples):
    ...
    ret = mx.nd.zeros(shape=(int(len(label)), 80, 8), ctx=mx.gpu())
#     ret = mx.nd.zeros(shape=(int(len(label)), 8, 80), ctx=mx.gpu())
